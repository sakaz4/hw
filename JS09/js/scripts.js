...
function processInput(event) {
    // функция выхывается когда в input воодят новое значение и нажимают enter
    let inputText = document.querySelector('.input').value; // достаем строку, которую ввели в input

    let listElement = document.createElement("li"); // создаем новый элемент списка
    listElement.innerHTML = "<input type='checkbox'><span>" + inputText + "</span>"; // устанавливаем ему нашу введенную строку в качестве текста
    listElement.onclick = editListItem; // привявзываем события клика на него функцию editListItem
